...
public:
    explicit PythonBindingGenerator(py::module_ &m) : module(m) {}
...
private:
    ...
    void bind_constructors(py::class_<T> &py_class, const TypeInfo &type_info)
    {
        ...
        // Bind each constructor using lambda with type erasure
                ...
                [ctor_ptr = ctor.get()](T &self, py::args args)
                {
                    ...
                    if (args.size() != ctor_ptr->parameter_types.size())
                    {
                        throw py::value_error(
                            "Constructor expects " +
                            std::to_string(ctor_ptr->parameter_types.size()) +
                    ...
                    {
                        cpp_args.push_back(
                            convert_python_to_any(args[i],
                                                  ctor_ptr->parameter_types[i]));
                    }

                    // Call factory to create object and placement-new it
                    void *raw_ptr = ctor_ptr->factory(cpp_args);
                    new (&self) T(std::move(*static_cast<T *>(raw_ptr)));
            ...
            py_class.def_property(
                ...
                [this, member_name](const T &obj) -> py::object
                {
                    try
                    {
                        ...
                        return convert_any_to_python(value,
                                                     obj.getTypeInfo()
                                                         .getMember(member_name)
                                                         ->type_name);
                    }
                    catch (const std::exception &e)
                    {
                        throw py::value_error("Failed to get member '" +
                                              member_name + "': " + e.what());
                    }
                },
                // Setter
                [this, member_name](T &obj, py::object py_value)
                {
                    try
                    {
                        ...
                        auto cpp_value = convert_python_to_any(
                            py_value, member_info->type_name);
                        obj.setMemberValue(member_name, cpp_value);
                    }
                    catch (const std::exception &e)
                    {
                        throw py::value_error("Failed to set member '" +
                                              member_name + "': " + e.what());
                    }
                },
                ...
                [this, method_name](T &obj, py::args args) -> py::object
                {
                    try
                    {
                        ...
                        const auto *method_info =
                            obj.getTypeInfo().getMethod(method_name);
                        ...
                        for (size_t i = 0; i < args.size(); ++i)
                        {
                            cpp_args.push_back(convert_python_to_any(
                                args[i], method_info->parameter_types[i]));
                        }
    ...
    void bind_introspection_utilities(py::class_<T> &py_class)
    {
        ...
        py_class.def("get_member_names", &T::getMemberNames,
                     "Get all member names");
        py_class.def("get_method_names", &T::getMethodNames,
        ...
        py_class.def(
            ...
            [this](T &obj, const std::string &name, py::object value)
            {
                ...
                auto cpp_value =
                    convert_python_to_any(value, member->type_name);
        ...
        py_class.def(
            ...
            [this](T &obj, const std::string &name, py::list args) -> py::object
            {
                ...
                for (size_t i = 0; i < args.size(); ++i)
                {
                    ...
                    {
                        cpp_args.push_back(convert_python_to_any(
                            args[i], method->parameter_types[i]));
                    }
                }
    ...
    py::object convert_any_to_python(const std::any &value,
                                     const std::string &type_name) const
    {
        ...
        {
            throw py::type_error("Failed to convert type '" + type_name +
                                 "' to Python: " + e.what());
        }
    }

    // Convert Python object to std::any based on expected type
    std::any convert_python_to_any(py::object py_value,
                                   const std::string &type_name) const
    {
        try
        {
            ...
            else
            {
                ...
                throw py::type_error("Unsupported type conversion for: " +
                                     type_name);
            }
        }
        catch (const py::cast_error &e)
        {
            throw py::type_error("Failed to convert Python object to '" +
                                 type_name + "': " + e.what());
        }
    }
};
...
#define PYBIND11_AUTO_BIND_CLASS(module, ClassName) \
    PythonBindingGenerator(module).bind_class<ClassName>(#ClassName)
